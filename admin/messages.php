    ...
    <div class="admin-container">
        <header style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-end;">
            <div>
                <p
                    ...
                    Communications</p>
                <h1 style="font-family: 'Playfair Display', serif; font-size: 2.5rem;">Message <span
                        class="text-accent">Inbox</span></h1>
            </div>
        ...
        <?php if ($success_msg): ?>
            <div class="alert alert-success"
                style="background: rgba(76, 175, 80, 0.1); color: #4caf50; padding: 1rem; margin-bottom: 1rem; border: 1px solid #4caf50; border-radius: 4px;">
                <?php echo $success_msg; ?></div>
        <?php endif; ?>
        <?php if ($error_msg): ?>
            <div class="alert alert-error"
                style="background: rgba(244, 67, 54, 0.1); color: #f44336; padding: 1rem; margin-bottom: 1rem; border: 1px solid #f44336; border-radius: 4px;">
                ...
                <table class="inbox-table">
                    ...
                    <tbody>
                        <?php if (empty($messages)): ?>
                            <tr>
                                <td colspan="5" style="text-align: center; padding: 3rem; color: #666;">No messages found.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($messages as $msg): ?>
                                <tr
                                    onclick='openMessage(<?php echo htmlspecialchars(json_encode($msg), ENT_QUOTES, 'UTF-8'); ?>)'>
                                    <td style="text-align: center;">
                                        <span class="msg-status status-<?php echo $msg['status'] ?? 'new'; ?>"
                                            title="<?php echo ucfirst($msg['status'] ?? 'new'); ?>"></span>
                                    </td>
                                    <td style="font-weight: 600;"><?php echo htmlspecialchars($msg['name']); ?></td>
                                    <td><?php echo htmlspecialchars($msg['subject']); ?></td>
                                    <td class="preview-text">
                                        <?php echo htmlspecialchars(substr($msg['message'], 0, 50)) . '...'; ?></td>
                                    <td style="color: #666; font-size: 0.85rem;">
                                        <?php echo date('M d', strtotime($msg['created_at'])); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
        ...
        <div id="msgModal" class="modal-overlay">
            <div class="modal-content glass-card">
                ...
                <div id="modalBody"
                    style="background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 4px; margin-bottom: 1.5rem; white-space: pre-wrap; max-height: 200px; overflow-y: auto;">
                    Message content...
                </div>
                ...
                <div id="replySection">
                    ...
                    <form method="POST">
                        ...
                        <textarea name="reply_body" rows="4" class="form-control" placeholder="Type your reply here..."
                            required
                ...
                <div id="repliedBadge"
                    style="display: none; text-align: center; padding: 1rem; background: rgba(76, 175, 80, 0.1); color: #4caf50; border: 1px solid #4caf50; border-radius: 4px;">
